...
3. The extension walks the currently visible result cards one at a time, uses deterministic rate-limited smooth scrolling with short adaptive waits, switches the official right-hand detail panel, and captures the full visible JD. When the requested amount exceeds the current card batch, it advances the left result list by roughly one card height, waits for lazy-loaded cards, deduplicates them, and continues until the requested limit is reached or the list stops producing new jobs. A card summary is retained only when a detail panel cannot be confirmed. CAPTCHA, login, platform-limit, or risk-control state stops collection rather than attempting to bypass it.
4. Each captured job is sent by the extension background in a small batch to `POST /api/platforms/boss/import-visible`; the task therefore continues and persists data even when the user navigates away from `/browser-tasks`. A lightweight progress summary is stored in extension local storage and restored when the page is reopened. The final capture response repairs any missed progress event. The API validates `zhipin.com` URLs, normalizes the JD, deduplicates by platform/external ID or content hash, and persists the job. Recollecting a previously summarized job with a full detail-panel JD upgrades the existing database record in place.
...
After a collection finishes, the page offers “清空本次结果” and “重新检索 BOSS”. These controls reset only the current workflow list, selections, confirmation, and progress display; persisted jobs and task history remain available in their respective pages.
...
- A single user-started automation request can specify 1–200 user-visible jobs. Large batches continue to emit and persist each job incrementally, and the page timeout is based on a lack of progress rather than total task duration. The actual total still depends on cards that BOSS makes available while the left result list is scrolled.
