  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      <header>
        ...
        <p className="mt-3 max-w-3xl text-slate-500">每个职位都有独立、可审计的状态历史；Phase 6 只排队，不执行真实网站投递。</p>
      </header>

      {error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-800">{error}</div>}

      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[["等待确认", counts.waiting], ["队列中", counts.queued], ["已提交", counts.submitted], ["需要处理", counts.attention]].map(([label, value]) => (
      ...
      <section className="panel overflow-hidden p-0">
        ...
        {loading ? <div className="px-6 py-12 text-center text-slate-500">加载中…</div> : applications.length === 0 ? <div className="px-6 py-12 text-center text-slate-500">还没有 Application。请先启动 Campaign 并批准职位。</div> : (
