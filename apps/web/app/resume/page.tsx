  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 whitespace-pre-wrap leading-7 text-slate-700">{profile.summary || "暂无摘要"}</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <ProfileGroup title="Experience" count={profile.experience.length}>
        ...
        <ProfileGroup title="Projects" count={profile.projects.length}>
        ...
        <ProfileGroup title="Education" count={profile.education.length}>
        ...
        <ProfileGroup title="Other" count={profile.awards.length + profile.publications.length}>
      ...
      {chunks.map((chunk, index) => (
        <article key={chunk.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span className="text-xs text-slate-500">embedding · {chunk.embedding_dimensions}d</span>
          </div>
          <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-600">{chunk.content}</p>
        </article>
      ))}
      {chunks.length === 0 && <p className="text-sm text-slate-400">暂无 Chunk。</p>}
    </div>
  );
}
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <p className="mt-3 text-slate-500">上传一次，保留原文、结构化 Profile、语义 Chunk 与向量索引。</p>
        </div>
        <span className="rounded-full bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-700">Phase 2</span>
      </header>
      ...
      <div className="grid gap-8 xl:grid-cols-[280px_1fr]">
        <section className="panel h-fit">
          ...
          <div className="mt-5 space-y-2">
            ...
            {!loading && resumes.length === 0 && <p className="text-sm leading-6 text-slate-400">还没有简历，先上传一份开始建立 Profile。</p>}
            {resumes.map((resume) => (
              <button key={resume.id} className={`w-full rounded-xl border p-4 text-left transition ${selected?.id === resume.id ? "border-indigo-300 bg-indigo-50" : "border-slate-200 hover:border-indigo-200 hover:bg-slate-50"}`} onClick={() => void selectResume(resume)} type="button">
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-slate-400">
                  <span>{resume.chunk_count} chunks</span>
          ...
          {selected ? (
            <>
              <section className="panel">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <button className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-indigo-300 hover:text-indigo-700 disabled:cursor-not-allowed disabled:opacity-50" disabled={parsing} onClick={() => void handleParse()} type="button">
                    {parsing ? "重新解析中…" : "重新解析"}
                  </button>
                </div>
              ...
              <section className="panel">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    <p className="eyebrow">Semantic Chunks</p>
                    <h2 className="mt-2 text-xl font-semibold">简历分块与向量</h2>
                  </div>
                  <span className="text-sm text-slate-500">{chunks.length} chunks</span>
                </div>
              ...
              <details className="panel group">
                ...
                <pre className="mt-4 max-h-96 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-950 p-4 text-sm leading-6 text-slate-200">{selected.raw_text}</pre>
              </details>
            </>
          ) : (
            <section className="panel grid min-h-80 place-items-center border-dashed text-center">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500">上传简历后，这里会展示 Profile、技能和 Chunk。</p>
              </div>
            </section>
          )}
        </div>
