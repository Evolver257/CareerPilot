...
export default function BrowserTasksPage() {
  ...
  function load() {
    return Promise.all([getBrowserTasks(), getApplications()]).then(([taskResponse, applicationResponse]) => {
      ...
      setApplicationId((current) => current || applicationResponse.items.find((item) => item.status === "QUEUED")?.id || "");
    });
  }
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      await createBrowserTask({ application_id: applicationId, scenario, auto_start: true });
      await load();
    } catch (reason) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <section className="panel">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">New Task</p><h2 className="mt-2 text-xl font-semibold">启动 Mock 投递</h2></div><Link className="text-sm font-medium text-indigo-700" href="/mock-platform">打开 Mock Job Site →</Link></div>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <label className="text-sm font-medium text-slate-700">已排队 Application<select className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 font-normal outline-none focus:border-indigo-500" onChange={(event) => setApplicationId(event.target.value)} value={applicationId}><option value="">请选择 Application</option>{applications.map((item) => <option key={item.id} value={item.id}>{item.job.title} · {item.id.slice(0, 8)}</option>)}</select></label>
          <label className="text-sm font-medium text-slate-700">Mock 场景<select className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 font-normal outline-none focus:border-indigo-500" onChange={(event) => setScenario(event.target.value)} value={scenario}><option value="SUCCESS">SUCCESS · 正常投递</option><option value="CAPTCHA_REQUIRED">CAPTCHA_REQUIRED · 需要人工处理</option><option value="LOGIN_REQUIRED">LOGIN_REQUIRED · 需要登录</option><option value="PLATFORM_LIMIT">PLATFORM_LIMIT · 平台限制</option><option value="DOM_CHANGED">DOM_CHANGED · DOM 变化</option></select></label>
        </div>
