...
export default function BrowserTaskDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      <header className="panel"><Link className="text-sm font-medium text-indigo-700" href="/browser-tasks">← Browser Tasks</Link><div className="mt-5 flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">Browser Task Trace</p><h1 className="mt-3 text-2xl font-semibold">{task.platform} Application Execution</h1><p className="mt-2 font-mono text-xs text-slate-400">{task.id}</p></div><span className="rounded-full bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700">{statusLabels[task.status]}</span></div><div className="mt-6 flex flex-wrap gap-3 border-t border-slate-100 pt-5">{task.status === "PENDING" && <button className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white" disabled={busy} onClick={() => void action("start")} type="button">Start</button>}{task.status === "CONNECTING" && <button className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white" disabled={busy} onClick={() => void simulate()} type="button">运行本地 Extension</button>}{task.status === "WAITING_FOR_USER" && <button className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-medium text-white" disabled={busy} onClick={() => void resume()} type="button">用户已处理，Resume</button>}{!terminal && <button className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700" disabled={busy} onClick={() => void action("cancel")} type="button">Cancel</button>}<Link className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-700" href={`/mock-platform/jobs/${String(task.payload.context && (task.payload.context as Record<string, unknown>).job_id)}?task=${task.id}`}>打开本地 Job Site</Link></div></header>
      {error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-800">{error}</div>}
      {task.status === "WAITING_FOR_USER" && <section className="panel border-amber-200 bg-amber-50/50"><p className="eyebrow text-amber-700">Human in the Loop</p><h2 className="mt-2 text-xl font-semibold">需要人工处理</h2><p className="mt-3 text-sm text-slate-600">{currentAction.metadata?.reason ?? task.failure_reason}</p><p className="mt-3 text-sm text-slate-500">处理验证码、登录、风控或未知页面后，点击 Resume 继续 Browser Task。</p></section>}
